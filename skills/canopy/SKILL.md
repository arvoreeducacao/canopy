...
- The daemon is shared by every agent session on the machine. Restarting it kills everyone's live sessions and tabs, not just yours — before any restart, call `browser_status` and make sure no other session is active. A failing call is usually the extension's service worker or a stale ref, not a dead daemon.
